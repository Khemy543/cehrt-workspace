    ...
    async getDashboardData() {
      try {
        ...
        if (response) {
          ...
          const vSubtasks = subtasks && subtasks.map((item) => {
            return {
              ...item,
              id: `subtask-${item.id}`,
              title: `${item.name} ( ${item.status})`,
              end: item.end_date,
              start: item.start_date || (item.created_at && item.created_at.split('T')[0]),
              className: item.priority.toLowerCase() === 'low' ? 'bg-success text-white' : item.priority.toLowerCase() === 'medium'? 'bg-warning text-white' : 'bg-danger text-white',
            } || []
          });

          const vTasks = tasks && tasks.map((item) => {
            return {
              ...item,
              id: `task-${item.id}`,
              title: `${item.name} ( ${item.status} )`,
          ...
          this.calendarEvents = [...vSubtasks, ...vTasks]
        }
      } catch (error) {
